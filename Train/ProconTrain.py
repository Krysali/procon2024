...
class PROCONNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=4, out_channels=64, kernel_size=3, padding=1)  # (256, 256, 4) -> (256, 256, 64)
        self.conv2 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, padding=1)  # -> (256, 256, 128)
        self.conv3 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, padding=1)  # -> (256, 256, 256)
        ...
        self.fc1 = nn.Linear(256 * 128 * 128, 1024)  # Input size: 256 channels * 128 * 128
...
def test():
    model = PROCONNet()
    input_tensor = torch.randn(1, 4, 256, 256)  # Batch size of 1, 4 channels, 256x256 input
...
def generate_training_data(game, num_samples):
    ...
    for _ in range(num_samples):
        ...
        for action, child_state in children:
            value = bfs_evaluate(game, child_state)
            if value > best_value:
                best_value = value
                best_action = action
...
for iteration in range(num_iterations):
    # Generate new batch of training data using ADI
    game = Game(model)
    training_data = generate_training_data(game, batch_size)
    
    states = []
    actions = []
    values = []

    for state, action, value in training_data:
        states.append(state)  # Append the board state
        actions.append(action)  # Append the optimal action
    ...
    # Compute policy loss
    die_loss = policy_loss_fn(outputs['die_probs'], actions[:, 0])
    x_loss = policy_loss_fn(outputs['x_probs'], actions[:, 1])
    y_loss = policy_loss_fn(outputs['y_probs'], actions[:, 2])
    direction_loss = policy_loss_fn(outputs['direction_probs'], actions[:, 3])

    # Compute value loss
    value_loss = value_loss_fn(outputs['heuristic_value'].squeeze(), values)

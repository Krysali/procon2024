...
GameState GameState::apply_die(Action action) {
    ...
    GameState nextState(res, goal_state);

    return nextState;
}

GameState GameState::raction(Action a) {
    // Setup
    std::vector<std::vector<int>> cboard = board.pieces;
    int n = board.n;
    int m = board.m;
    ...
    if (dice_type == 1) {
        return reverseTypeI(a.x, a.y, size, a.direction, n, m, cboard, goal_state);
    } else if (dice_type == 2) {
        return reverseTypeII(a.x, a.y, size, a.direction, n, m, cboard, goal_state);
    } else {
        return reverseTypeIII(a.x, a.y, size, a.direction, n, m, cboard, goal_state);
    }
}

GameState reverseTypeI(int X, int Y, int size, int s, int n, int m, std::vector<std::vector<int>> board, Board goal_state)
{
    int otx = std::max(X, 0);
    int oty = std::max(Y, 0);
...
GameState reverseTypeII(int X, int Y, int size, int s, int n, int m, std::vector<std::vector<int>> board, Board goal_state)
{
    int otx = std::max(X, 0);
    int oty = std::max(Y, 0);
    int obx = std::min(X + size - 1, m - 1);
...
GameState reverseTypeIII(int X, int Y, int size, int s, int n, int m, std::vector<std::vector<int>> board, Board goal_state)
{
    int otx = std::max(X, 0);
    int oty = std::max(Y, 0);
    int obx = std::min(X + size - 1, m - 1);

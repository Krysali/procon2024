...
void GameState::compute_heuristics(){
    h += columncheck(board , goal_state, board.height, board.width);
    h += rowcheck(board , goal_state, board.height, board.width);
	return;
}

void GameState::setter(GameState* parent, Action action, int g){
    this -> parent = parent;
    this -> action = action;
    this -> g = g;
    f = g + h;
...
void GameState::print_path(){
	this -> display_game_state();
	while(parent != nullptr){
		parent -> display_game_state();
		std::cout << '\n';

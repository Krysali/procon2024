...
int main() {
    ...
    torch::load(model, "model_0.pt");  
    torch::Tensor input = torch::rand({1, 8, 128, 128}); // Adjust dimensions as necessary

    // Get only the heuristic value
    auto heuristic_value = model.forward(input, true);
    // heuristic_value 
    parallelWeightedAStar(env, depthPenalty, numParallel ,  heuristic_value);

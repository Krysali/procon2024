...
void reverseTypeI(int X, int Y, int size, int s)
{
    ...
    int xstart = max(X, 0);
    int xend = min(X + w - 1, m);
    int ystart = min(Y, 0);
    int yend = min(Y + h - 1, n);
        ...
        switch (s)
        {
            case 0:
                for(int index_x = xstart; index_x < xend; index_x++){
                    ...
                    for(int index_y = ystart; index_y < n; index_y++){
                        if(board[index_y][index_x] != -1){
                            board[last_empty][index_x] = board[index_y][index_x];
                            last_empty++;
                        }
                    }
                    for(int index_y = last_empty; index_y < h; index_y++){
            ...
            case 2:
                for(int index_y = ystart; index_y < yend; index_y++){
                    ...
                    for(int index_x = xstart; index_x < m; index_x++){
                        if(board[index_y][index_x] != -1){
                            board[index_y][last_empty] = board[index_y][index_x];
                            last_empty++;
                        }
                    }
                    for(int index_x = last_empty; index_x < m; index_x++){
                        board[index_y][index_x] = -1;
                    }
                }
                break;
